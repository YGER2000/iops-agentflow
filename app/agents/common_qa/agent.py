...
# from app.core.models import SharedConversationHistory, SharedConversationHistoryMongo
...
class CommonQAAgent(AgentBase):
    ...
    def __init__(self):
        super().__init__(
            name="common_agent",
            description="通识问答智能体,可以回答各种常识性问题,支持多轮对话"
        )
    ...
    def build_graph(self) -> StateGraph:
        ...
        workflow = build_common_qa_graph(llm_service=self.llm)
    ...
    async def invoke(
        ...
        #     logger.debug("MongoDB: 用户消息已保存到共享集合")
        # except Exception as e:
        #     logger.error(f"MongoDB 保存用户消息失败: {e}")
        ...
        #         )
        #         await save_document(collection, conversation)
        #         logger.debug("MongoDB: AI回复已保存到共享集合")
        # except Exception as e:
        #     logger.error(f"MongoDB 保存AI回复失败: {e}")
        #
